...
import com.ververica.flink.table.config.entries.CatalogEntry;
import com.ververica.flink.table.config.entries.ConfigurationEntry;
...
import com.ververica.flink.table.config.entries.FunctionEntry;
import com.ververica.flink.table.config.entries.GatewayEntry;
import com.ververica.flink.table.config.entries.ModuleEntry;
import com.ververica.flink.table.config.entries.SessionEntry;
...
public class Environment {
	...
	public static final String GATEWAY_ENTRY = "gateway";
	...
	public Environment() {
		...
		this.gateway = GatewayEntry.DEFAULT_INSTANCE;
	}
	...
	public void setGateway(Map<String, Object> config) {
		this.gateway = GatewayEntry.create(config);
	}

	public GatewayEntry getGateway() {
		return gateway;
	}
	...
	public String toString() {
		...
		sb.append("==================== Gateway =====================\n");
		gateway.asTopLevelMap().forEach((k, v) -> sb.append(k).append(": ").append(v).append('\n'));
		return sb.toString();
	}
	...
	public static Environment merge(Environment env1, Environment env2) {
		...
		// merge gateway properties
		mergedEnv.gateway = GatewayEntry.merge(env1.getGateway(), env2.getGateway());
	...
	public static Environment enrich(
		...
		// does not change gateway properties
		enrichedEnv.gateway = env.getGateway();
