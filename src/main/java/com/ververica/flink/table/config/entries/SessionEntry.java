...
import static com.ververica.flink.table.config.Environment.SESSION_ENTRY;
...
public class SessionEntry extends ConfigEntry {
	...
	protected void validate(DescriptorProperties properties) {
		properties.validateString(SESSION_NAME, true);
		properties.validateDuration(SESSION_IDLE_TIMEOUT, true, 1, 1800_000); // min: 30m
		properties.validateDuration(SESSION_CHECK_INTERVAL, true, 1, 600_000); // min 10m
		properties.validateLong(SESSION_MAX_COUNT, true, 1);
	}
	...
	public Optional<String> getSessionName() {
		return properties.getOptionalString(SESSION_NAME);
	}
