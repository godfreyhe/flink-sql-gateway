...
package com.ververica.flink.table.config.entries;

import com.ververica.flink.table.config.ConfigUtil;
...
import static com.ververica.flink.table.config.Environment.GATEWAY_ENTRY;

/**
 * Describes a gateway configuration entry.
 */
public class GatewayEntry extends ConfigEntry {

	public static final GatewayEntry DEFAULT_INSTANCE = new GatewayEntry(new DescriptorProperties(true));
	...
	private GatewayEntry(DescriptorProperties properties) {
		super(properties);
	}
	...
	public static GatewayEntry create(Map<String, Object> config) {
		return new GatewayEntry(ConfigUtil.normalizeYaml(config));
	}

	public Map<String, String> asTopLevelMap() {
		return properties.asPrefixedMap(GATEWAY_ENTRY + '.');
	}
	 ...
	 */
	public static GatewayEntry merge(GatewayEntry gateway1, GatewayEntry gateway2) {
		...
		return new GatewayEntry(properties);
	}
