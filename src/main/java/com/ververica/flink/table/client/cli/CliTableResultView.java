...
import com.ververica.flink.table.client.result.MaterializedBatchResult;
import com.ververica.flink.table.client.result.MaterializedResult;
import com.ververica.flink.table.client.result.MaterializedStreamResult;
...
public class CliTableResultView extends CliResultView<CliTableResultView.ResultTableOperation> {
	...
	public CliTableResultView(CliClient client, JobID jobId) {
		super(client, jobId);
	...
	protected TableSchema getTableSchema() {
		return materializedResult.getTableSchema();
	}
	...
	protected List<AttributedString> computeMainHeaderLines() {
		...
		Arrays.stream(getTableSchema().getFieldNames()).forEach(s -> {
			schemaHeader.append(' ');
