...
import com.ververica.flink.table.client.cli.SqlCommandParser.SqlCommand;
...
public final class CliStrings {
	...
	public static final AttributedString MESSAGE_HELP = new AttributedStringBuilder()
		...
		.append(formatCommand(SqlCommand.CREATE_VIEW,
		...
		.append(formatCommand(SqlCommand.DROP_VIEW,
			"Deletes a previously created virtual table. Syntax: 'DROP VIEW <name>;'"))
		.append(
			formatCommand(SqlCommand.EXPLAIN, "Describes the execution plan of a query or table with the given name."))
		.append(formatCommand(SqlCommand.HELP, "Prints the available commands."))
		.append(formatCommand(SqlCommand.INSERT_INTO,
			"Inserts the results of a SQL SELECT query into a declared table sink."))
		.append(formatCommand(SqlCommand.INSERT_OVERWRITE,
		...
		.append(formatCommand(SqlCommand.SET,
		...
		.append(formatCommand(SqlCommand.SOURCE,
			"Reads a SQL SELECT query from a file and executes it on the Flink cluster."))
		.append(formatCommand(SqlCommand.USE_CATALOG,
	...
	public static final AttributedString INPUT_HELP = new AttributedStringBuilder()
		...
		.append("ESC")
		.style(AttributedStyle.DEFAULT)
		.append(" or submit an empty string to cancelJob.")
		.toAttributedString();
	...
	public static AttributedString messageError(String message, Throwable t) {
		...
		return messageError(message, extractExceptionMessage(t));
	...
	private static String extractExceptionMessage(Throwable t) {
		...
		if (t instanceof RestClientException) {
			...
			if (originalMessage.contains(RestHandlerException.class.getName())) {
				return t.getClass().getName() + ": " +
					originalMessage.substring(1, t.getMessage().indexOf(RestHandlerException.class.getName()) - 2);
			} else {
				...
				for (int i = lines.length - 1; i >= 0; i--) {
					...
					int causeByLen = "Caused by: ".length();
		...
		return t.getClass().getName() + ": " + originalMessage;
	}
