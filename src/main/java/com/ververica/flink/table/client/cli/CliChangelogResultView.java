...
import com.ververica.flink.table.client.result.ChangelogStreamResult;
...
import static com.ververica.flink.table.client.cli.CliUtils.TIME_FORMATTER;
import static com.ververica.flink.table.client.cli.CliUtils.formatTwoLineHelpOptions;
import static com.ververica.flink.table.client.cli.CliUtils.normalizeColumn;
import static com.ververica.flink.table.client.cli.CliUtils.repeatChar;
import static com.ververica.flink.table.client.cli.CliUtils.rowToString;
...
public class CliChangelogResultView extends CliResultView<CliChangelogResultView.ResultChangelogOperation> {
	...
	private ChangelogStreamResult changelogStreamResult;

	public CliChangelogResultView(CliClient client, JobID jobId) {
		super(client, jobId);
	...
	protected TableSchema getTableSchema() {
		return changelogStreamResult.getTableSchema();
	}
	...
	protected void refresh() {
		...
		try {
			result = changelogStreamResult.retrieveChanges();
	...
	protected List<AttributedString> computeHeaderLines() {
		...
		final String left;
		if (isRetrieving()) {
			left = CliStrings.DEFAULT_MARGIN + CliStrings.RESULT_REFRESH_INTERVAL + ' ' + REFRESH_INTERVALS
				.get(refreshInterval).f0;
		} else {
		...
		if (lastRetrieval == null) {
			...
			right = CliStrings.RESULT_LAST_REFRESH + ' ' + lastRetrieval
				.format(TIME_FORMATTER) + CliStrings.DEFAULT_MARGIN;
		}
		// all together
	...
	protected List<AttributedString> computeMainHeaderLines() {
		...
		Arrays.stream(changelogStreamResult.getTableSchema().getFieldNames()).forEach(s -> {
			schemaHeader.append(' ');
			schemaHeader.style(AttributedStyle.DEFAULT.underline());
