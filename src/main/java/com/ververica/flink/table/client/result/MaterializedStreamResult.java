 ...
 */
public class MaterializedStreamResult extends StreamResult implements MaterializedResult {
	...
	public MaterializedStreamResult(SessionClient session, JobID jobId, int maxTableResultRows) {
		...
		if (maxTableResultRows <= 0) {
			...
			this.maxRowCount = maxTableResultRows;
		}

		this.overcommitThreshold = computeMaterializedTableOvercommit(maxRowCount);
