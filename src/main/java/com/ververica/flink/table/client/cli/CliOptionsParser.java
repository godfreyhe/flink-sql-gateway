...
public class CliOptionsParser {
	...
	public static final Option OPTION_GATEWAY_SERVER_HOST = Option
		.builder("a")
		.required(true)
		.longOpt("address")
		.numberOfArgs(1)
		.type(Integer.class)
		.argName("gateway server address")
		.desc("gateway server address")
		.build();

	public static final Option OPTION_GATEWAY_SERVER_PORT = Option
		.builder("p")
		.required(false)
		.longOpt("port")
		.numberOfArgs(1)
	...
	public static Options getGatewayModeClientOptions(Options options) {
		buildGeneralOptions(options);
		options.addOption(OPTION_GATEWAY_SERVER_HOST);
		options.addOption(OPTION_GATEWAY_SERVER_PORT);
		options.addOption(OPTION_ENVIRONMENT);
		options.addOption(OPTION_UPDATE);
		return options;
	}
	...
	public static void printHelpGatewayModeClient() {
		...
		System.out.println("\nMode \"gateway\" connects to the SQL gateway for submission.");
	...
	public static CliOptions parseGatewayModeClient(String[] args) {
		try {
			...
			if (line.hasOption(CliOptionsParser.OPTION_GATEWAY_SERVER_PORT.getOpt())) {
				gatewayPort = Integer
					.valueOf(line.getOptionValue(CliOptionsParser.OPTION_GATEWAY_SERVER_PORT.getOpt()));
			}
			return new CliOptions(
				line.hasOption(CliOptionsParser.OPTION_HELP.getOpt()),
				line.getOptionValue(CliOptionsParser.OPTION_GATEWAY_SERVER_HOST.getOpt()),
				gatewayPort,
				checkUrl(line, CliOptionsParser.OPTION_ENVIRONMENT),
				null,
				checkUrls(line, CliOptionsParser.OPTION_JAR),
