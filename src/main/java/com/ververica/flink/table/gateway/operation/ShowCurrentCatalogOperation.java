...
public class ShowCurrentCatalogOperation implements NonJobOperation {
	...
	public ShowCurrentCatalogOperation(String sessionId, Executor executor) {
	...
	public ResultSet execute() {
		return OperationUtil.singleStringToResultSet(
			((ExecutorImpl) executor).getExecutionContext(sessionId).getTableEnvironment().getCurrentCatalog(),
			ConstantNames.CATALOG);
	}
}
