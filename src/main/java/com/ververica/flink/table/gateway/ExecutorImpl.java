...
import com.ververica.flink.table.gateway.result.ResultStore;
...
public class ExecutorImpl implements Executor {
	...
	private final ConcurrentHashMap<String, Set<String>> sessionResultMap;
	private final ConcurrentHashMap<String, String> sessionNameMap;
	 ...
	 */
	public ExecutorImpl(Environment defaultEnvironment, List<URL> jars, List<URL> libraries) {
		...
		this.sessionResultMap = new ConcurrentHashMap<>();
		this.sessionNameMap = new ConcurrentHashMap<>();
	...
	public String openSession(SessionContext sessionContext) throws SqlExecutionException {
		...
		if (this.contextMap.containsKey(sessionId)) {
			...
			this.sessionResultMap.put(
				sessionId, ConcurrentHashMap.newKeySet());
			if (sessionContext.getSessionName().isPresent()) {
				this.sessionNameMap.put(sessionId, sessionContext.getSessionName().get());
			}
		}
		return sessionId;
	}
	...
	public void closeSession(String sessionId) throws SqlExecutionException {
		for (String resultId : sessionResultMap.get(sessionId)) {
			try {
	...
	public ResultDescriptor executeQuery(String sessionId, String query) throws SqlExecutionException {
		final ExecutionContext<?> context = getExecutionContext(sessionId);
		ResultDescriptor descriptor = executeQueryInternal(sessionId, context, query);
		sessionResultMap.get(sessionId).add(descriptor.getResultId());
		return descriptor;
	}
	...
	public void cancelQuery(String sessionId, String resultId) throws SqlExecutionException {
		final ExecutionContext<?> context = getExecutionContext(sessionId);
		sessionResultMap.get(sessionId).remove(resultId);
	...
	private <C> ProgramTargetDescriptor executeUpdateInternal(
		...
		final String sessionName = sessionNameMap.get(sessionId);
		final String jobName = sessionName != null ? sessionName + ": " + sessionId + ": " + statement : sessionId + ": " + statement;
	...
	private <C> ResultDescriptor executeQueryInternal(String sessionId, ExecutionContext<C> context, String query) {
		...
		final String sessionName = sessionNameMap.get(sessionId);
		final String jobName = sessionName != null ? sessionName + ": " + sessionId + ": " + query : sessionId + ": " + query;
	 ...
	 */
	private <C> void applyUpdate(ExecutionContext<C> context, TableEnvironment tableEnv, QueryConfig queryConfig,
