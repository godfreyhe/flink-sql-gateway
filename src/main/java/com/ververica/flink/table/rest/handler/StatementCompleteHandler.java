...
/**
 * Handler for statement complete hints.
 */
public class StatementCompleteHandler extends AbstractRestHandler<
	StatementCompleteRequestBody, StatementCompleteResponseBody, SessionMessageParameters> {

	private final SessionManager sessionManager;

	public StatementCompleteHandler(
		SessionManager sessionManager,
		Time timeout,
		Map<String, String> responseHeaders,
		MessageHeaders<
			StatementCompleteRequestBody,
			StatementCompleteResponseBody,
	...
	@Override
	protected CompletableFuture<StatementCompleteResponseBody> handleRequest(
		@Nonnull HandlerRequest<StatementCompleteRequestBody, SessionMessageParameters> request)
		...
		Integer position = request.getRequestBody().getPosition();
		if (position == null) {
			throw new RestHandlerException("position must be provided.", HttpResponseStatus.BAD_REQUEST);
		}

		try {
			ResultSet resultSet = sessionManager.getSession(sessionId).completeStatement(statement, position);
			return CompletableFuture.completedFuture(new StatementCompleteResponseBody(resultSet));
